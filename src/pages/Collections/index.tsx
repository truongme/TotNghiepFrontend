        ...
        <div className='collections-ctn'>
            <div className='container'>
                <div className='collections-breadcrumb'>
                    <Link to={'/'} className='link-style collections-breadcrumb-item'>Home</Link>
                    <div className='collections-breadcrumb-item'>/</div>
                    <Link to={'/'} className='link-style collections-breadcrumb-item'>COLLECTIONS</Link>
                    <div className='collections-breadcrumb-item'>/</div>
                ...
                <div className='list-subcatgory'>
                    <div className='container d-flex'>
                        {subCategory.list.map((e : any) =>(
                            <div className='list-subcatgory-item' onClick={() => navigate(`/collections/${e.link}`)}>
                                <div className='list-subcatgory-img-ctn'>
                                    <img src={ e.img || "https://brand.assets.adidas.com/image/upload/f_auto,q_auto,fl_lossy/PAC_FW_23_CYBER_WEEK_MEMBERS_EA_PLP_IWP_MEN_D_d9f2721ad9.jpg"} alt="" />
                                </div>
                                <div className='list-subcatgory-name'>
                                    {e.name}
                                </div>
                            </div>
                        ))}
                    ...
                    <div className='list-category-bg'>
                        <img src="https://brand.assets.adidas.com/image/upload/f_auto,q_auto,fl_lossy/01_fw23_brand_campaign_launch_plp_superstar_iwp_bg_d_2c58860564.jpg" alt="" />
                    </div>
                </div>
            )}
            <div className='container'>
                <div className='collections-header'>
                    <div className='collections-filter-button' onClick={() => setIsOpenPannel(true)}>
                        Filter +
                    </div>
                    <div className='collections-actions-sort'>
                        <div>Sort by</div>
                        <button className={`collections-sort-btn ${btnSort === 'popularity' && "active"}  `} onClick={() => sortProducts('popularity')}>Best Seller</button>
                        <button className={`collections-sort-btn ${btnSort === 'ascending' && "active"}  `} onClick={() => sortProducts('ascending')}>Ascending</button>
                        <button className={`collections-sort-btn ${btnSort === 'descending' && "active"}  `} onClick={() => sortProducts('descending')}>Descending</button> 
                    </div>
                </div>
                        ...
                        <div className='pannel-filter-container'>
                            ...
                            <div className='pannel-filter-content'>
                                    ...
                                    <div >
                                        ...
                                        <div className='pannel-filter-subCategory-ctn'>
                                            {subCategory.list.map(e => (
                                                <div className={`pannel-filter-subCategory ${listSelectedSubCategory.includes(e.link) ? 'selected': ''}`} onClick={() => handleSelectSubCategory(e.link)}>{e.name}</div>
                                            ))}
                                        </div>
                                    </div>
                                )}
                                ...
                                <div className='pannel-filter-sizes'>
                                    {arrSize.map(x => (
                                        <div className={listSelectedSize.includes(x) ? 'pannel-filter-size selected' : 'pannel-filter-size'} onClick={() => handleSelectSize(x)}>
                                            {x}
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className='pannel-filter-colors'>
                                    {listColor.map(e => (
                                        <div key={e.id} className={`pannel-filter-color ${listSelectedColor.includes(e.id) ? 'selected' : ''}`} style={{background:`${e.hexCode}`}} onClick={() => handleSelectColor(e.id)}>
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className='pannel-filter-prices'>
                                    {arrFilterPrice.map(x => (
                                        <div 
                                            className={`pannel-filter-price ${listSelectedPrice.some(p => p.min === x.min && p.max === x.max) ? 'selected' : ''}`} 
                            ...
                            <div className='pannel-filter-footer'>
                                <button className='btn-secondary' onClick={() => handleClearAllFilter()}>Clear All</button>
                                <button className='btn-primary' onClick={() => setIsOpenPannel(false)}>Apply</button>
                            </div>
                        </div>
                    </div>
                )}
