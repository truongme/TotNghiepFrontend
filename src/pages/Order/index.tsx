...
const Order = () => {
  ...
  const handleAddressChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (selectedId === "new") {
      ...
      const address: any = arrMyAddresses?.find(
        (addr: any) => addr.addressId == selectedId
      );
      if (address) {
  ...
  const fetchDistricts = async (id: number) => {
    try {
      const response = await axios.get(
        `https://open.oapi.vn/location/districts/${id}?size=1000`
      );
  ...
  const fetchWard = async (id: number) => {
    try {
      const response = await axios.get(
        `https://open.oapi.vn/location/wards/${id}?size=1000`
      );
  ...
  const onSubmit = async (data: OrderForm) => {
    try {
      if (data.paymentMethod == "MOMO") {
        ...
        setCart(true);
        await axios.put(
          `${WebUrl}/api/v1/orders/complete-order`,
          {
  ...
  const createPay = async () => {
    try {
      const res: any = await axios.post(
        `${WebUrl}/api/v1/payments/momo`,
        {
          amount: totalOrder + shippingCost,
          orderId: "222212",
        },
        {
          headers: {
      ...
      console.error("Error post item payments", error);
    }
  };
    ...
    <div className="container mb-5">
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="order-breadcrumb" style={{ paddingTop: "20px" }}>
          <Link to={"/"} className="link-style order-breadcrumb-item">
            Trang chủ
          </Link>
          <div className="order-breadcrumb-item">/</div>
          <Link to={"/cart"} className="link-style order-breadcrumb-item">
            Giỏ hàng
          </Link>
          <div className="order-breadcrumb-item">/</div>
        ...
        <div className="row" style={{ marginTop: "20px" }}>
          <div className="col-7">
            <div className="order-user">
              <div className="order-user-title">Thông tin người nhận</div>
                    ...
                    <Controller
                      ...
                      render={({ field }) => {
                        ...
                        return (
                          <div>
                            <div
                              className={`form-check-ship ${
                                value === "SHIPMENT" ? "selected" : ""
                              }`}
                              onClick={() => {
                                setValue("shipmentMethod", "SHIPMENT");
                                setShippingCost(city === "1" ? 15000 : 30000);
                              }}
                            >
                              <div className="ship-method-header">
                                <div>Tiêu chuẩn</div>
                                <div>
                                  {formatPrice(city === "1" ? 15000 : 30000)}
                                </div>
                              </div>
                              <div className="ship-method-detail">
                                Đảm bảo giao hàng trong vòng 2 đến 3 ngày
                              </div>
                            </div>
                            <div
                              className={`form-check-ship ${
                                value === "EXPRESS" ? "selected" : ""
                              }`}
                              onClick={() => {
                                setValue("shipmentMethod", "EXPRESS");
                            ...
                            >
                              ...
                              <div className="ship-method-detail">
                                Đảm bảo giao hàng vào ngày mai
                              </div>
                            </div>
                          </div>
                        );
          ...
          <div className="col-5">
            ...
            <div className="order-items">
              ...
              <div className="d-flex justify-content-between">
                <div>Tổng tiền</div>
                <p className="m-0 p-0">
                  {formatPrice(totalOrder + shippingCost)}
                </p>
              </div>
              <hr />
      ...
      {isOpenModalExit && (
        <ModalMain
          title="Thông báo"
          content="Bạn có muốn thoát khỏi trang thanh toán đơn hàng không?"
          btn1="No"
          btn2="Yes"
          onClose={handleCancelCloseModalExit}
          onSave={handleCloseModalExit}
        />
      )}
    </div>
  );
